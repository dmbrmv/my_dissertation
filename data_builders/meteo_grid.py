...
cmip_models_245 = [grid_descriptor(dataset_name=model,
                                   half_resolution=0.125,
                                   files=Path(model_path))
                   for model, model_path in cmip_models_245.items()]
cmip_models_585 = {model.split('/')[-1]: model
                   for model in glob.glob(f'{cmip_585}/*')}
cmip_models_585 = [grid_descriptor(dataset_name=model,
                                   half_resolution=0.125,
                                   files=Path(model_path))

...
from meteo_grids_parser.scripts.geom_proc import area_from_gdf, poly_from_multipoly

from .hydro_atlas_variables import (
    climate_variables,
    hydrology_variables,
    landcover_variables,
    monthes,
...
def featureXtractor(user_ws: Polygon, gdb_file_path: str):
    ...
    gdf = gpd.read_file(gdb_file_path, mask=user_ws, layer=layer_small, ignore_geometry=False)
    ...
    gdf["gdf_geometry"] = [
        gpd.GeoDataFrame({"geometry": sample_geom}, index=[0]).set_crs("EPSG:4326")
        for sample_geom in gdf["gdf_geometry"]
    ]
    # calculate weight of each intersection correspond to it native size
    gdf["weights"] = gdf["gdf_geometry"].apply(lambda x: gpd.overlay(gdf_your_ws, x)).apply(
        area_from_gdf
    ) / gdf["gdf_geometry"].apply(area_from_gdf)
    # calculate area with weight appliance
    gdf["weight_area"] = tuple(map(area_from_gdf, gdf["gdf_geometry"])) * gdf["weights"]
    ...
    geo_vector = gdf[
        ...
        + soil_and_geo_variables
        + urban_variables
    ].applymap(lambda x: np.sum(x * gdf["weights"]) / np.sum(gdf["weights"]))
    ...
    divide_by_100 = [
        item
        for sublist in [["ari_ix_sav"], ["cmi_ix_s{}".format(i) for i in monthes]]
        for item in sublist
    ]

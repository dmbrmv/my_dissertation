...
h_path = glob.glob("../data/great_db/ais_data/levels_xls/*.xls")

id_lbl_asso = list()
for f_path in tqdm(h_path):
    h_name = f_path.split("/")[-1][:-4]
    id_lbl_asso.append(level_to_csv(data_path=f_path, save_path=Path(f"./data_2/lvl_csv/{h_name}")))
...
file_extender(
    files_by_district=glob.glob("./data_2/lvl_district_csv/*/*.csv"),
    save_storage=Path("./data_2/res/levels"),
)

q_path = glob.glob("../data/great_db/ais_data/discharge_xls/*.xls")
...
file_extender(
    files_by_district=glob.glob("./data_2/q_district_csv/*/*.csv"),

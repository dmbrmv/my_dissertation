...
def network_creator(tile_tag: str):
    ...
    lons = np.arange(start=min_lon, stop=max_lon, step=2.5)
    lats = np.arange(start=min_lat, stop=max_lat, step=2.5)
    # write tiles from whole raster in predefined place
    acc_tiles_storage = Path(f"./data/river_network/rasters/{tile_tag}")
    ...
    for lon_index in range(len(lons) - 2):
        ...
        for lat_index in range(len(lats) - 2):
            ...
            tile_counter += 1
            tile_acc.rio.to_raster(f"{acc_tiles_storage}/{tile_tag}_{tile_counter}.tiff")
...
def river_separator(tile_tag: str):
    geometry_storage = Path(f"./data/river_network/geometry/{tile_tag}")
    geometry_storage.mkdir(exist_ok=True, parents=True)
    # get preselected tiles
    tiles_to_network = glob.glob(f"./data/river_network/rasters/{tile_tag}/*.tiff")
    ...
    for tile in tiles_to_network:
        ...
        temp_res_riv = gpd.GeoDataFrame(
            pd.concat([get_river_geom(rank, network) for rank, network in river_parts.items()]),
            geometry="geometry",
        )  # type: ignore

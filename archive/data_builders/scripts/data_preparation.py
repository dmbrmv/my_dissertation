...
from .geom_functions import min_max_xy, round_down, round_up


def aoi_name_path(shp_generator):
    name_n_path = {str(path).split("/")[-1][:-4]: path for path in shp_generator}
...
def tile_selector(topo_p, gauge_gdf):
    x_min, y_min, x_max, y_max = (
        gauge_gdf[["x", "y"]].describe().loc[["min", "max"], ["x", "y"]].values.ravel()
    )
    ...
    tile_boundaries = [
        ...
        for lat, lon in product(
            np.arange(start=y_min, stop=y_max, step=5), np.arange(start=x_min, stop=x_max, step=5)
        )
    ]
    variables = ["dir", "elv"]
...
def aoi_tiles(topo_p: Union[Path, str], aoi_shp: str) -> dict:
    ...
    if (x_min < 0) & (x_max < 0):
        ...
        tile_boundaries = [
            f"n{lat}w0{lon}" if len(str(lon)) < 3 else f"n{lat}w{lon}"
            for lat, lon in product(
                np.arange(start=y_min, stop=y_max, step=5), np.arange(start=x_min, stop=x_max, step=5)
            )
        ]
...
def tile_determinator(gauge_file, path_to_save):
    all_tiles = glob.glob(f"{config_info.mask_storage}/*.gpkg")
    ...
    for tile, ids in gauge_file.groupby("target_tile").groups.items():
        ...
        gauge_file.loc[ids, :].to_file(f"{path_to_save}/{fname}.gpkg", encoding="utf-8")

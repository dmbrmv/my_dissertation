...
for gauge_id in tqdm(gauges):
    ...
    metric_res_df = metric_df(gauge_id=gauge_id, predictions=res_df["pred"], targets=res_df["obs"])
    metric_res_df.index.name = "gauge_id"
    metric_res_df.to_csv(f"./res/multi/{gauge_id}.csv")
    gc.collect()
    torch.cuda.empty_cache()

tft_single_gauge_stat = pd.concat(tft_single_gauge_stat)
tft_single_gauge_stat.to_csv("./some_tft_res_multi.csv")

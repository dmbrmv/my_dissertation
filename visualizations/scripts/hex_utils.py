...
def build_hex_grid(extent_poly: Polygon, r_km: float = 75.0, crs: str = EQ_AREA_CRS) -> gpd.GeoDataFrame:
...
def aggregate_nse_to_hex(
    ...
    joined = gpd.sjoin(
        w_cent[[nse_col, "geometry"]], hexes.reset_index(names="hex_id"), how="inner", predicate="within"
    )
...
def summarize_hex_coverage(hexes: gpd.GeoDataFrame, count_col: str = "count") -> dict[str, float]:

...
def read_netcdf(
    net_cdf_on_disk: Union[str, list],
) -> Union[
    Tuple[Hashable, xr.DataArray, xr.DataArray, xr.DataArray],
    Tuple[Hashable, None, None, xr.DataArray],
]:
    """_summary_
    ...
    dataset = xr.open_mfdataset(net_cdf_on_disk, chunks="auto")
    ...
    if variable_name in ["t2m"]:
        ...
        return (variable_name, None, None, varible_data_sum)
    else:
        varible_data_mean = varible_data.resample(time="1D").mean()
        return (variable_name, None, None, varible_data_mean)


def nc_by_extent(nc: xr.Dataset, shape: Polygon, grid_res: float, dataset: str = ""):
    """
    ...
    min_lon, max_lon, min_lat, max_lat = find_extent(
        ws=big_shape, grid_res=grid_res, dataset=dataset
    )
